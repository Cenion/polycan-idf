...
ERROR_t MCP2515_init(MCP2515 *MCP2515_Object) {
	...
	*MCP2515_Object = (MCP2515)malloc(sizeof(MCP2515_t[1]));
    ...
    ESP_LOGI(TAG_MCP2515, "Allocated %d bytes for MCP2515_Object at %p", sizeof(MCP2515_t[1]), &MCP2515_Object);
...
ERROR_t MCP2515_setFilterMask(MCP2515 *MCP2515_Object, const MASK_t mask, const bool ext, const uint32_t ulData)
{
    ...
    uint8_t tbufdata[4];
    MCP2515_prepareId(MCP2515_Object, tbufdata, ext, ulData);
...
ERROR_t MCP2515_sendMessage(MCP2515 *MCP2515_Object, const TXBn_t txbn, const CAN_FRAME frame)
{
    ...
    MCP2515_prepareId(MCP2515_Object, data, ext, id);
